...
\centerline{
	...
	\begin{algorithmic}[1]
		...
		\ENSURE $\mathbf{z} = (z_1, z_2, \cdots, z_{2N})$
        \FOR{$i = 2N$ \TO $1$}
            \STATE $t_i' \gets t_i + \sum_{j>i}\mathbf{U}_{ij}(t_j - z_j)$
            \STATE $\sigma_i \gets \frac{\sigma}{\| \mathbf{\tilde b}_i \|}$ \qquad { // $\mathbf{\tilde b}_i$ is the $i$-th row vector of $\mathbf{\tilde B}$}
			\STATE $z_i \getsdollar D_{\Z,\sigma_i,t_i'}$ \qquad {// Sample a value $z_i$ from a discrete Gaussian distribution}
