...
FPN multiplication (FprMul) is proceeded by
    ...
    \only<5->{\item Right-shifting the mantissa }
    \only<6->{\item Packing and rounding (FPR)}
...
FPN addition (FprAdd) is proceeded by
    ...
    \only<10->{ \item Normalizing the result}
    \only<11->{\item Packing and rounding (FPR)}
...
\begin{algorithm}[H]
  ...
  \begin{algorithmic}[1]
	...
	\STATE \textcolor{red}{Normalize $z, ex$ to make $z \in [2^{63}, 2^{64})$ \label{alg:FprAdd:normalize}}
	\STATE $z \gets (z \gg  9) \vee \textcolor{red}{\llbracket z^{[9:1]} \neq 0 \rrbracket}$
    \STATE $ex \gets ex + 9$ \\
