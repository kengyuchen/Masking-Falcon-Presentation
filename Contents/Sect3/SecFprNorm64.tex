...
\]
\[
\text{where } c \text{ is the smallest integer such that } \bigoplus_{i=1}^n x_i \ll c \in [2^{63}, 2^{64})
\]
\pause

We repeatedly check whether $(x^{(64)}_i)$ is $0$ or not, then conditionally shift it by $1$ bit, and then decrease $(e_i)$ by $\llbracket (x^{(64)}_i) = 0 \rrbracket$.
\pause
\medskip
